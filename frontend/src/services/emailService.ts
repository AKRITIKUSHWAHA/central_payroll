...
class EmailService {
  public async sendPayslipEmail(payload: SendEmailPayload): Promise<{ success: boolean; message: string }> {
    ...
    const apiRes = await apiFetch<{ success: boolean; message?: string; error?: string }>('/email/payslip', {
      method: 'POST',
      body: JSON.stringify({
    ...
    if (!apiRes || !apiRes.success) {
      return {
        success: false,
        message: apiRes?.error || 'Failed to dispatch email. Check SMTP configuration in backend.'
      };
    }
    
    // Store outbox item in localStorage
