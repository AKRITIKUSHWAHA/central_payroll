...
export const Employees: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setEmployees((data || []).filter(isRealEmployee));
    } catch (err) {
      console.error(err);
  ...
  const openEditModal = (emp: Employee) => {
    ...
    setFormData({
      ...
      payRate: emp.payRate !== undefined ? emp.payRate : 16.50,
      holidayRate: emp.holidayRate !== undefined ? emp.holidayRate : 24.75,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const regRate = Number(formData.payRate) >= 0 ? Number(formData.payRate) : 0;
      const holRate = Number(formData.holidayRate) >= 0 ? Number(formData.holidayRate) : Math.round(regRate * 1.5 * 100) / 100;
          ...
          <table className="w-full text-left text-sm border-collapse min-w-[760px]">
            <thead>
              <tr className="bg-[#102a43] text-white font-bold text-xs uppercase tracking-wider">
                ...
                <th className="py-3.5 px-5 font-bold">Pay Rate</th>
                ...
                employees.map((emp) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-5 font-bold text-[#0f172a] whitespace-nowrap">
                      <div>{emp.displayName || `${emp.firstName} ${emp.lastName}`.trim()}</div>
                      <div className="text-[11px] font-normal text-[#64748b]">{emp.position || 'Staff Member'}</div>
                    </td>

                    {/* Pay Rates */}
                    <td className="py-3.5 px-5 whitespace-nowrap">
                      <div className="text-xs font-black text-[#0f766e] tabular-nums">
                        ${(Number(emp.payRate) || 0).toFixed(2)}/hr
                      </div>
                      <div className="text-[11px] font-bold text-[#64748b] tabular-nums">
                        Hol: ${(Number(emp.holidayRate) || (Number(emp.payRate) || 0) * 1.5).toFixed(2)}/hr
                      </div>
                    </td>
          ...
          <div className="bg-white border border-[#d7e2ec] rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div className="p-3 bg-[#ecfdf5] rounded-xl border border-[#a7f3d0]">
                <div className="flex items-center gap-1.5 text-[#047857] font-bold mb-1">
                  <span>Regular Rate (Hourly)</span>
                </div>
                <div className="text-sm font-black text-[#065f46] tabular-nums">
                  ${(Number(viewModalEmployee.payRate) || 0).toFixed(2)}/hr
                </div>
              </div>

              <div className="p-3 bg-[#f0fdfa] rounded-xl border border-[#99f6e4]">
                <div className="flex items-center gap-1.5 text-[#0f766e] font-bold mb-1">
                  <span>Holiday Rate</span>
                </div>
                <div className="text-sm font-black text-[#115e59] tabular-nums">
                  ${(Number(viewModalEmployee.holidayRate) || (Number(viewModalEmployee.payRate) || 0) * 1.5).toFixed(2)}/hr
                </div>
              </div>
          ...
          <div className="bg-white border border-[#d7e2ec] rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto animate-fadeIn">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-[#334155] mb-1">Regular Rate ($/hr) *</label>
                  <input
                    type="number"
                    step="0.25"
                    min="0"
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({
                        ...formData,
                        payRate: val,
                        holidayRate: !isNaN(num) && num > 0 ? (Math.round(num * 1.5 * 100) / 100).toFixed(2) : formData.holidayRate
                      });
                    }}
                    className="w-full px-3.5 py-2 border border-[#cbd5e1] rounded-xl text-sm font-semibold focus:outline-none focus:border-[#1d4ed8]"
                  />
              ...
              <div>
                <label className="block text-xs font-bold text-[#334155] mb-1">Holiday Rate ($/hr)</label>
                <input
                  type="number"
                  step="0.25"
                  min="0"
