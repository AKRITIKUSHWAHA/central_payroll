...
class StaffContactDetailsService {
  private getLocalRecords(): Record<string, StaffRecordItem[]> {
    ...
    try {
      ...
      // Clean out customer/invalid staff IDs from records
      const cleaned: Record<string, StaffRecordItem[]> = {};
      Object.keys(parsed).forEach(k => {
        if (!k.toLowerCase().startsWith('cust_') && !['tanuvi', 'cust_416', 'cust_417'].includes(k.toLowerCase())) {
          cleaned[k] = parsed[k];
        }
      });
      return cleaned;
  ...
  public async getStaffContactDetails(): Promise<Employee[]> {
    try {
      ...
      if (res && res.success && res.contacts) {
        return res.contacts.filter(isRealEmployee);
      }
    } catch (err) {
      console.warn('Failed to fetch from /staff-contact-details:', err);
    }
    return employeeService.getEmployees().filter(isRealEmployee);
  }
