...
export const LeaveCalendar: React.FC = () => {
  ...
  const [selectedEmployeeId, setSelectedEmployeeId] = useState<string>('alesia');
  ...
  useEffect(() => {
    leaveService.fetchLeaveData().then(data => {
      const validEmps = (data.employees || []).filter(isRealEmployee);
      if (validEmps.length > 0) {
        setEmployees(validEmps);
        if (!selectedEmployeeId || !validEmps.some(e => e.id === selectedEmployeeId || e.employeeId === selectedEmployeeId)) {
          setSelectedEmployeeId(validEmps[0].id || validEmps[0].employeeId);
        }
      }
  ...
  const selectedEmployee = useMemo(() => {
    return (
      employees.find(e => e.id === selectedEmployeeId || e.employeeId === selectedEmployeeId) || {
        id: selectedEmployeeId || 'global',
        employeeId: selectedEmployeeId || 'CDL-002',
