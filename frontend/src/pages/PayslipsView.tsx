...
export const PayslipsView: React.FC = () => {
  const [employees, setEmployees] = useState<Employee[]>(() => {
    ...
    return list.filter((e: Employee) => isRealEmployee(e) && (e.status || 'Active') !== 'Inactive');
  });
  const [periods, setPeriods] = useState<PayrollPeriod[]>(() => {
    return payrollService.getPayrollPeriods();
  });
  const [selectedEmpId, setSelectedEmpId] = useState<string>(() => {
    const list = employeeService.getEmployees().filter((e: Employee) => isRealEmployee(e) && (e.status || 'Active') !== 'Inactive');
    return list[0]?.id || '';
  });
  ...
  useEffect(() => {
    ...
    // 1. Fetch latest employees from employee service
    employeeService.fetchEmployees().then((list: Employee[]) => {
      if (list && list.length > 0) {
        const activeEmps = list.filter((e: Employee) => isRealEmployee(e) && (e.status || 'Active') !== 'Inactive');
        setEmployees(activeEmps);
        setSelectedEmpId(prev => prev || activeEmps[0]?.id || '');
      }
    });

    // 2. Fetch latest payslips & periods from API
    apiFetch<{ success: boolean; payslipPeriods: PayrollPeriod[]; employees: Employee[] }>('/payslips').then(res => {
      if (res && res.success) {
        if (res.employees && res.employees.length > 0) {
          const activeEmps = res.employees.filter((e: Employee) => isRealEmployee(e) && (e.status || 'Active') !== 'Inactive');
          setEmployees(activeEmps);
          setSelectedEmpId(prev => prev || activeEmps[0]?.id || '');
        }
  ...
  const currentPeriod = (periods && periods.length > 0) ? periods[0] : payrollService.getCurrentDraft();
  ...
  const item = currentPeriod?.items?.find(i => i.employeeId === selectedEmp?.employeeId || i.employeeId === selectedEmp?.id || i.employeeName === selectedEmp?.displayName) || currentPeriod?.items?.[0];
