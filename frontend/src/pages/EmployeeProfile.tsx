...
export const EmployeeProfile: React.FC = () => {
  ...
  const [editPayRate, setEditPayRate] = useState<number | string>(employee?.payRate || 16.50);
  const [editHolidayRate, setEditHolidayRate] = useState<number | string>(employee?.holidayRate || 24.75);
      ...
      <div className="bg-white border border-[#dde7f0] rounded-2xl p-6 shadow-cdCard flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="bg-[#f4f7fb] border border-[#dde7f0] rounded-xl p-4 flex items-center gap-6">
          ...
          <div className="border-l border-[#c9d7e6] pl-6">
            ...
            <div className="text-sm font-black text-[#0f766e] tabular-nums flex items-center gap-2">
              <span>{showSensitive ? `$${employee.payRate.toFixed(2)}/hr` : '••••••••'}</span>
              <button
                type="button"
                onClick={() => {
                  setEditPayRate(employee.payRate);
                  setEditHolidayRate(employee.holidayRate || employee.payRate * 1.5);
                  setIsEditingRates(true);
                }}
                className="px-2 py-0.5 bg-[#e0f2fe] hover:bg-[#bae6fd] text-[#0369a1] text-[11px] font-bold rounded-lg border border-[#7dd3fc] transition-all"
              >
                Edit Rate
              </button>
            </div>
          ...
          <div className="bg-white border border-[#d7e2ec] rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4 animate-fadeIn">
            <div className="flex items-center justify-between border-b border-[#e2e8f0] pb-3">
              <h3 className="text-lg font-black text-[#12345b]">
                Edit Pay Rates • {employee.displayName}
              </h3>
              <button
            ...
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                const regRate = parseFloat(String(editPayRate)) || 0;
                const holRate = parseFloat(String(editHolidayRate)) || Math.round(regRate * 1.5 * 100) / 100;
                ...
                try {
                  ...
                  showToast(`Pay rates updated for ${employee.displayName}.`);
                  setIsEditingRates(false);
                } catch (err) {
                  showToast('Failed to update pay rates.', 'error');
                } finally {
            ...
            >
              <div>
                <label className="block text-[#334155] mb-1">Regular Hourly Rate ($/hr) *</label>
                <input
                  type="number"
                  step="0.25"
                  min="0"
                  ...
                  onChange={(e) => {
                    ...
                    setEditPayRate(val);
                    if (!isNaN(num) && num > 0) {
                      setEditHolidayRate((Math.round(num * 1.5 * 100) / 100).toFixed(2));
                    }
                  }}
              ...
              <div>
                <label className="block text-[#334155] mb-1">Holiday Rate ($/hr)</label>
                <input
                  type="number"
                  step="0.25"
                  min="0"
                  value={editHolidayRate}
                  onChange={(e) => setEditHolidayRate(e.target.value)}
                  className="w-full px-3.5 py-2 border border-[#cbd5e1] rounded-xl text-sm font-semibold focus:outline-none focus:border-[#1d4ed8]"
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-[#e2e8f0]">
                <button
                  type="button"
                  onClick={() => setIsEditingRates(false)}
                  className="px-4 py-2 border border-[#cbd5e1] text-[#475569] rounded-xl hover:bg-[#f8fafc]"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-5 py-2 bg-[#1d4ed8] hover:bg-[#1e40af] text-white rounded-xl shadow-md disabled:opacity-50"
                >
